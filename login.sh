...
#reduced form
checkoutput=$(curl -s "${web_address}" -H "Origin: $web_address" -H "Referer: $web_address/fgtauth?$secure_key" --data "4Tredir=http%3A%2F%2Fwww.gstatic.com%2Fgenerate_204&magic=$secure_key&username=$username&password=$password" --compressed)
