...
You can install the files to your system by using the `install.sh` script, once installed it will add aliases for `login.sh` as `nitc-login` and `logout.sh` as `nitc-logout`. You can then also see the content inside the log_file by the command `nitc-logfile`. 
